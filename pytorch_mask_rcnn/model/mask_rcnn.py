...
class MaskRCNN(nn.Module):
    ...
    def __init__(self, backbone, num_classes,
                 ...
                 box_score_thresh=0.33, box_nms_thresh=0.5, box_num_detections=100):
        super().__init__()
        ...
        trajectory_predictor = TrajectoryPredictorNet(in_channels, mid_channels, num_classes)
        
        self.head = RoIHeads(
             box_roi_pool, box_predictor, trajectory_predictor,
    ...
    def forward(self, image, target=None, time=None):
        ...
        result, roi_losses = self.head(feature, proposal, image_shape, target, time)
...
class TrajectoryPredictorNet(nn.Module):
    def __init__(self, in_channels, mid_channels, num_classes):
        super().__init__()
        self.fc1_a = nn.Linear(1, mid_channels//3)
        self.fc1_b = nn.Linear(4*num_classes, mid_channels//3)
        self.fc1_c = nn.Linear(in_channels, mid_channels//3 + 1)
        self.fc2 = nn.Linear(mid_channels, mid_channels)
        self.traj_pred = nn.Linear(mid_channels, num_classes * 4) # delta_t = 1, delta_boxes
        
    def forward(self, x, boxes, time_step):
        time_step = F.relu(self.fc1_a(time_step.repeat(x.shape[0], 1)))
        boxes = F.relu(self.fc1_b(boxes))
        x = x.flatten(start_dim=1)
        x = F.relu(self.fc1_c(x))
        x = torch.cat((x, boxes, time_step), 1)
        x = F.relu(self.fc2(x))
        traj_delta = self.traj_pred(x)
...
def maskrcnn_resnet50(pretrained, num_classes, pretrained_backbone=True):
    ...
    if pretrained:
        ...
        for i, name in enumerate(msd):
            if i in skip_list or 'trajectory' in name:
                continue
